...
- 이전 기술
  ...
  - `JSP`(Java Server Page) : HTML과 Java소스를 분리. 쉽게 개발하도록 만든 기술(난이도 중)
    - 개발환경 구성 난이도가 높음
  - `Spring` : Java개발 전성기. 웹페이지와 Java영역 분리. 개발환경 구성 난이도 줄어듦
    - 개발환경 구성 난이도 중
    - 대한민국 전자정부 웹프레임워크 개발
...
12. Gradle, Java 빌드 진행 - [소스](./day03/demo/src/main/java/com/pknu26/demo/DemoApplication.java)
    - 빌드 이후 작업표시줄 `Java: Ready`가 표시되어야 함
    - `Java: Error`는 프로젝트 생성 실패. 새로 구성
...
- build.gradle : 자바버전, 플러그인, 의존성 설정 파일 - [소스](./day03/demo/build.gradle)
  ...
  // Gradle 플러그인 설정
   ...
   // 라이브러리 의존성 설정
   dependencies {
      implementation 'org.springframework.boot:spring-boot-starter-web'	// Spring MVC 구현용
      testImplementation 'org.springframework.boot:spring-boot-starter-test'	// 테스트 구현용
...
- Spring MVC 구조
  ...
  - 브라우저 요청 -> Controller 호출 -> Model에 데이터 담고(Service) -> View 반환 -> 요청한 브라우저에 돌려줌

- Spring MVC 구현
  1. Service (MessageService.java) 생성 - [소스](./day03/springmvc/src/main/java/com/pknu26/springmvc/Service/MessageService.java)
  2. Controller (HomeController.java) 생성 : 기능별로 여러 컨트롤러 생성 가능 - [소스](./day03/springmvc/src/main/java/com/pknu26/springmvc/Controller/HomeController.java)
  3. View (src/main/resources/templates/home.html) 생성 - [소스](./day03/springmvc/src/main/resources/templates/home.html)
  4. 기본 흐름 : Controller -> Service -> View 순서로 구성
  5. 주의사항 : 소스코드 작성 및 수정 이후에는 반드시 서버를 '재시작'해야 반영됩니다.

- 어노테이션(Annotation) 목록
  - @SpringBootApplication : 스프링 부트 설정 클래스 (수정 금지)
...
- 로그 사용법 - [소스](./day03/springlog/src/main/java/com/pknu26/springlog/HomeController.java)
  - Controller, Service, Repository 클래스에서 사용가능
...
- Choose dependencies 선택할 의존성 - [소스](./day05/webboard/build.gradle)
  ...
  - Spring Data JPA : DB와 ORM 연동 라이브러리
...
4. controller, entity, repository, service 폴더(패키지) 생성 - [소스](./day05/webboard/src/main/java/com/pknu26/webboard/)
...
- @RequiredArgsConstructor : `final` 멤버변수 파라미터를 생성자로 생성하는 Lombok 어노테이션
