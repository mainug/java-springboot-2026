...
- 아닌 경우는
  ...
  - Adoptium JDK - https://adoptium.net/temurin/releases
...
- src/test/java : 테스트 코드 위치 폴더. Java는 코딩과 테스트 동시에 진행
...
- Kotlin : Java를 대체하는 언어. Java 기반. Kotlin은 안드로이드/iOS 동시에 개발 가능
...
- 새로 공부한다 보다는 필요한 것만 보충해서 학습
- Javascript는 자바를 따라서 만들었다? Java와 Javascript는 문법적으로 다름
...
- 절차지향 방식의 문제점
  ...
  - 관련된 데이터가 흩어지고,
  - 같은 형태의 변수선언이 반복되고,
  - 학생 한 명의 정보를 하나의 단위(그룹)로 다루기 힘들고
...
- 생성자 특징
  ...
  - 반환형이 없음
  - 객체 초기화에만 사용
  - 객체 만들 때 필수 데이터들을 강제화
  - 잘못된 상태의 객체 생성을 줄임

- 매개변수 생성자 (추가 생성자)
  - 초기화 시 인자를 받아서 객체 생성하는 방법이 필요할 때 사용
  - 코딩양을 줄일 수 있고, 간단함

- this 키워드
  - Python의 self와 유사한 기능을 함
...
- 어노테이션 종류
  ...
  - @SuppressWarnings : 컴파일러에게 경고 메시지 출력하지 말것
...
- 자식이 반드시 구현해야 하는 메서드들을 강제할 수 있음
- 클래스들 틀에 맞춰서 생성하고 필요한 메서드도 `규칙을 정하겠다`는 의지
- 규칙대로 따르라는 설계 지침
...
```java
// iCage 인터페이스를 가져다 쓰는 클래스는 반드시 아래의 메서드들을 구현해야함
interface iCage {
    void checkIn();
    void checkOut();
}
